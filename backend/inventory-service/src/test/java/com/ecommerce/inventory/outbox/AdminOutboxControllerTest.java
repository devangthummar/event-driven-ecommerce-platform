...
import com.ecommerce.inventory.controller.AdminOutboxController;
...
class AdminOutboxControllerTest {
    ...
    void testRetryFailedOutboxMessages_Success() {
        when(outboxService.retryFailedOutboxMessages()).thenReturn(2);

        ResponseEntity<Map<String, Object>> response = adminOutboxController.retryFailedOutboxMessages();

        assertThat(response.getStatusCode()).isEqualTo(HttpStatus.OK);
        assertThat(response.getBody()).isNotNull();
        assertThat(response.getBody().get("retriedCount")).isEqualTo(2);
